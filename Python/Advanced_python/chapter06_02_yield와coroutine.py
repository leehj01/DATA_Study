...
# 코루틴 예제 5( yeild from ) - 3. 대에서 나온것
# StopIteration 자동처리 (3.7  이후부터는 ->  await 로 바뀜)
# 중첩 코루틴 처리

def gen1(): # 중첩으로 하나의 코루틴 안에 들어갈 수 있음
    for x in 'AB':
        yield x
    for y in range(1,4):
        yield y
...
print('EX 5-1 -', next(t1))
print('EX 5-2 -', next(t1))
print('EX 5-3 -', next(t1))
print('EX 5-4 -', next(t1))
print('EX 5-5 -', next(t1))
# print('EX 5-6 -', next(t1))  # StopIteration

t2 = gen1()

print('EX 5-7 -', list(t2))

print()
print()

def gen2():
    yield from 'AB' # 처음과 끝을 자기가 알고있고, 제어를 해줌 yield from 에 위임
    yield from range(1,4)

t3 = gen2()  # for 문으로 하는 것을 간단하게, yield from 으로 만들 수 있다.
print('EX 6-1 -', next(t3))
print('EX 6-2 -', next(t3))
print('EX 6-3 -', next(t3))
print('EX 6-4 -', next(t3))
print('EX 6-5 -', next(t3))

t4 = gen2()

print('EX 6-6 -', list(t4))
print()
print()

# 최종 예제 - 메인과 서브 테스크 - 흐름 제어
...
def gen4_main():
    yield from gen3_sub() # gen3_sub를 위임
    # 코루틴 끼리, yield from 을 통해서 흐름 제어를 할 수 있음

t5 = gen4_main()

print('EX 7-1 -', next(t5))
print('EX 7-2 -', t5.send(7))
print('EX 7-2 -', t5.send(77))
