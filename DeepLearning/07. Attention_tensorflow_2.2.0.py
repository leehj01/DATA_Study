...
# 인코더 단에, return_sequnences를 true로 만들어줘야함 .
class Encoder(tf.keras.Model):
    ...
    def call(self, x, training = False , mask = None):
        x = self.emb(x)
        H, h , c = self.lstm(x)
        return H, h, c # 모든 Hidden state를 sequnce형태로 출력을 해주기 위해 true를 한것 
...
class Decoder(tf.keras.Model):
    def __init__(self):
        ...
        self.att = tf.keras.layers.Attention() # attention 매커니즘을 사용해줌
    ...
    def call(self, inputs, training = False, mask = None):
        ...
        # y_ : shifted output, s0 : decoder의 hiddenstate - 처음들어와서 0 붙임
        # c0 : 처음 들어오는  cell state 것 
        # H : 인코더단에서 나온 hidden state 
        
        x = self.emb(x)  # 입력이 들어온 것을 임베딩 해줌 
        
        S, h, c = self.lstm(x, initial_state=[s0,c0]) 
        
        # 인코더로 들어온 것을 initial_state로 넣어주고 lstm을 돌림 
        ...
        # s0 는 처음 입력받은 부분이라 맨 앞에 들어가야함. length 가 1이기 때문에 2차원임
        # 3차원으로 확대하기 위해서 중간에 tf.newaxis 를 넣어줌. 
        # 이것과 붙여주는 것은 마지막 hiddenstate는 다음에 출력을 내지 않으므로 마지막은 배제
        
        
        # attention 적용 
        A = self.att([S_, H ]) # s h f 이런식으로 3개 쓰면 key와 value를 다르게 입력하지만
        # 나는 두개만 사용 h : 키와 벨류를 같은 것으로 사용
        
        y = tf.concat([S, A], axis = -1 )
        # attention value를 dense로 넣어주기 전에, dense로 들어가기로 된 heddenstate엤다가 cocnat을 해서 들어가게 함
...
class Seq2seq(tf.keras.Model):
    ...
    def call(self, inputs , training = False , mask = None):
        if training is True:  # 학습 과정
            ...
            H, h, c = self.enc(x) # 인코더에 입력(x)을 넣어서, 히든 state와 cell state를 받아즘 
            y, _, _ = self.dec((y, h, c, H))  # 디코더의 결과로 y가 됨. - 전체 문장이 됨 
            return y
...
"""문장이 끊기는 이유는 토크나이즈할 때, 높은 빈도수 2000개로 하여서 , 
문장이 끊긴채로 결과가 나온다 ."""
